...
def tempo_para_percorrer_uma_distancia(distancia, velocidade):
    ...
    vel= float(velocidade)
    return round(dist / vel, 2)
...
def aumento_salarial(salario, porcentagem):
    ...
    aumento_total= float((porcentagem + 100) / 100)
...
def preco_com_desconto(preco_original, percentual_desconto):
    ...
    desconto= float((percentual_desconto / 100) * preco_original)
...
def dias_perdidos_por_fumar(cigarros_fumados_por_dia, anos_fumando):
    ...
    """
    cigarros_fumados_por_dia= float(cigarros_fumados_por_dia)
    anos_fumando= float(anos_fumando)
    minutos_no_ano= 3650
    dias_no_ano= (minutos_no_ano / 1440)
    dias_perdidos= float((cigarros_fumados_por_dia * dias_no_ano)* anos_fumando)
    dias_perdidos= round(dias_perdidos,2)
    return float(dias_perdidos)
...
def dois_elevado_a_um_milhao():
    ...
    """
    dois_no_milhao = int(2 ** 1000000)
    dois_no_milhao = len(str(dois_no_milhao))
    return (dois_no_milhao)
...
def media_final_aprovado_reprovado(p1, p2, ep1, ep2):
    ...
    media= float((parcial_1 + parcial_2) /2)
    if media >= (7) :
        bool(True)
        return(True)
    else :
...
def tinta(metros_pintar):
    ...
    """
    import math
    metros_pintar= float(metros_pintar)
