...
def dias_perdidos_por_fumar(cigarros_fumados_por_dia, anos_fumando):
    ...
    """
    return round(((cigarros_fumados_por_dia*365*anos_fumando*10)/60/24),2)
...
def media_final_aprovado_reprovado(p1, p2, ep1, ep2):
    ...
    """
    return (((p1*7+ep1*3)/10+(p2*7+ep2*3)/10)/2 >= 7)
...
def salario(valor_hora, horas_mensais):
    ...
    """
    return valor_hora*horas_mensais - 0.24*valor_hora*horas_mensais
...
def decompor_numero(numero):
    ...
    """
    return (int(numero/100), int((numero%100)/10), int((numero%100)%10))
